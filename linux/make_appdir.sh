...
cp "${BUNDLE_DIR}/ntsapp" "$APPDIR/usr/bin/${APP_EXECUTABLE}"
...
for size in 16 24 32 48 64 128 256 512; do
  ...
  convert "$ICON_PATH" -resize ${size}x${size} "$APPDIR/usr/share/icons/hicolor/${size}x${size}/apps/${APP_EXECUTABLE}.png"
done
...
# === Create .desktop file ===
cat > "$APPDIR/${APP_EXECUTABLE}.desktop" <<EOF
[Desktop Entry]
Name=${APP_NAME}
...
cp "$APPDIR/${APP_EXECUTABLE}.desktop" "$APPDIR/usr/share/applications/"
